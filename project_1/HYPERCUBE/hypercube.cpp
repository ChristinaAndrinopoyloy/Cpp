...
vector<edge> hypercube(dataset &mydata, string method, int k, myMap &mymap)
{
    ...
    edge temp;
    vector<int> id;
    int i, j, m, h, empty_index;
    bool b;
    bool flag = 0; //find or not point at a "bucket"
    stringstream key;

    for (i = 0; i < mydata.get_number_of_points(); i++) //for each data
    {
        flag = 0;
        for (j = 0; j < k; j++)
        {
        ...
        for (m = 0; m < pow(2, k); m++)
        {
            ...
            {
                flag = 1;
                myhypercube[m].set_point(make_pair(i, mydata.get_data(i)));
            }
        ...
        {
            temp.set_id(key.str());
            temp.set_point(make_pair(i, mydata.get_data(i)));
            myhypercube[empty_index] = temp;
        }
        id.clear();
        key.str(string());
    }
